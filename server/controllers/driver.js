...
const listDriverTrips = async (req, res) => {
  try {
    ...
    const driverTrips = await db.tripModel.findAll({
...
const driverTripDetails = async (req, res) => {
  try {
    ...
    const driverTripDetail = await db.tripModel.findOne({
      where: { DriverId: driverId, id: tripId },
      include: [{ model: db.carModel }],
    });
